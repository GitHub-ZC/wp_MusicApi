...
接口：`/v1/qq/login_scan_long/`

示例：`http://iecoxe.top:3000/v1/qq/login_scan_long`

说明：本接口会默认返回二维码，用手机QQ扫码确认即可登录，自动部署Cookie，为防止恶意调用访问接口，默认 **限制访问间隔** 90秒，重复调用会覆盖服务端Cookie
